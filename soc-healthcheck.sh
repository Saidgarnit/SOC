...
EP="${ELASTIC_PASSWORD:-sYVfKJCe2RCfELjf=GLa}"
...
IDX=$(curl -sf -u "elastic:${EP}" "http://localhost:9200/_cat/indices?h=index" 2>/dev/null | grep -cv "^\." || echo 0)
ok "ES indices: $IDX"

hdr "3. WAZUH"
if docker ps --format "{{.Names}}" | grep -q wazuh-manager; then
  ok "Wazuh manager UP"
  ACT=$(docker exec wazuh-manager /var/ossec/bin/agent_control -l 2>/dev/null | grep -c Active || echo 0)
  TOT=$(docker exec wazuh-manager /var/ossec/bin/agent_control -l 2>/dev/null | grep -c "ID:" || echo 0)
  [ "$ACT" -ge 5 ] && ok "Agents: $ACT/$TOT active" || warn "Agents: $ACT/$TOT active"
  ALR=$(docker exec wazuh-manager wc -l /var/ossec/logs/alerts/alerts.json 2>/dev/null | awk "{print \}")
  ok "Wazuh alerts: $ALR"
...
ENR=$(curl -sf -u "elastic:${EP}" "http://localhost:9200/soc-logs-enriched-*/_count" 2>/dev/null | grep -o '"count":[0-9]*' | grep -o '[0-9]*' || echo 0)
ok "Enriched docs: $ENR"
RID=$(curl -sf -u "elastic:${EP}" "http://localhost:9200/soc-logs-enriched-*/_count" -H 'Content-Type: application/json' -d '{"query":{"exists":{"field":"rule.id"}}}' 2>/dev/null | grep -o '"count":[0-9]*' | grep -o '[0-9]*' || echo 0)
...
ok "Rules: $RC"
ER=$(docker logs elastalert --tail 50 2>&1 | grep -c ERROR || echo 0)
...
SA=$(curl -sf -u "elastic:${EP}" "http://localhost:9200/suricata-alerts-*/_count" 2>/dev/null | grep -o '"count":[0-9]*' | grep -o '[0-9]*' || echo 0)
...
KH=$(curl -sf -o /dev/null -w "%{http_code}" http://localhost:5601/api/status 2>/dev/null || echo 0)
